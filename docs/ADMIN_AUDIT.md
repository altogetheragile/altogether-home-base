...
| **`/admin/moderation`** | Orphaned (not in nav), "view in context" link is broken post-migration, inert unless KB comment-reporting is live. | S to remove — **confirm KB commenting is dead first**. |
...
- **Shared admin `DataTable`** — today there are 4+ table idioms (Blog inline-edit+dnd, Taxonomy tabbed, SEO sortable `SortHeader`, Knowledge `divide-y` list). One component with sort/filter/empty/loading/error/row-actions collapses them. Biggest UX lever.
...
- **Safe CSV util** — AdminUsers + AdminContacts hand-roll CSV with no escaping (commas/newlines corrupt the file). Extract `toCsv()`.
