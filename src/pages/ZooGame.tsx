    ...
    switch (state.phase) {
      ...
      case 'sprint':
        return <ZooShell state={state} {...shellProps} tools={<BoardTools state={state} teachCard={cardFor('sprint')} onMarkTaught={markTaught}
          onSetScrumAt={setDailyScrumAt} onSetLearnMode={setLearnMode} onSetWipLimit={setWipLimit} onCancelSprint={cancelSprint} />}><SprintBoard state={state} onAddAnother={addAnotherPbi} onEstimate={estimate} onToggleTask={toggleTask} onConfirmAc={confirmAc} onSendBack={sendBack} onFinishItem={edit.onFinishBuild} onStartItem={startItem} onCancelSprint={cancelSprint} onReorderSprint={reorderSprint} onSetLearnMode={setLearnMode} onSetWipLimit={setWipLimit} onSetScrumAt={setDailyScrumAt} onPull={pull} onDropFromSprint={dropFromSprint} onAnswerPlacement={answerPlacement} onOpen={deployComplete} onPlaceOnPark={placeOnParkAndEnter} onEndDay={endDay} onHoldDailyScrum={holdDailyScrum} onAnswerImpediment={answerImpediment} onSkipDailyScrum={skipDailyScrum} onStartDay={beginDay} onHoldRefinement={holdRefinement} onSplitEpic={splitEpic} building={buildingId} edit={edit} part={partFocus} onPart={setPartFocus} drawing={drawing} onDrawing={setDrawing} onRemoveRun={deleteConnector} onAddPbi={createPbi} onSetUserStories={setUserStories} onAddProposal={handleProposal} onDeclineProposal={declineProposal} onAssignDev={assignDev} onRenameMember={renameMember} onBuilding={selectOnPark} seat={seat} canBuild={!seat || seat === 'developer' || (covering ?? []).includes('developer')} teachCard={cardFor('sprint')} onMarkTaught={markTaught} /></ZooShell>;
      case 'review':
        return <ZooShell state={state} {...shellProps}><SprintReview state={state} onTakeSignal={takeSignal} onDeclineSignal={declineSignal} onContinue={() => setPhase('retro')} onWrapUp={() => setPhase('final')} onOpen={open} onConfirmAc={confirmAc} onToggleTask={toggleTask} teachCard={cardFor('review')} onMarkTaught={markTaught} /></ZooShell>;
      case 'retro':
