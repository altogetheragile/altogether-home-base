...
export default function ZooGame() {
  const { state, start, setPhase, setGoal, takeSignal, plan, pull, build, open, closeDay, holdDailyScrum, skipDailyScrum, nextSprint, reset } = useZooGame();
  ...
  const render = () => {
    switch (state.phase) {
      ...
      case 'sprint':
        return <SprintBoard state={state} onBuild={build} onPull={pull} onOpen={open} onEndDay={closeDay} onHoldDailyScrum={holdDailyScrum} onSkipDailyScrum={skipDailyScrum} />;
      case 'review':
