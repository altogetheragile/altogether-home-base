...
export function ZooGameScreens({ game, saves = true, seat = null, observer, covering, away, mustAgree = [], said, onDismissSaid, refused, onDismissRefused, onReading }:
  { game: ZooGameApi; saves?: boolean; seat?: SeatName | null; observer?: boolean; covering?: SeatName[];
    /** Seats held by somebody who is not connected right now. A subset of `covering`: their work
     *  falls to whoever is here, and the band says which of the two it is. */
  ...
  // Dropping a thing on the park is the moment it exists: whatever was in the draft becomes the
  // item's design. Without this the draft stayed a draft for ever - the Product Owner could not
  // accept it (there was nothing built to accept) and it could never reach Done.
  ...
  const render = () => {
    switch (state.phase) {
      ...
      case 'refine':
        return <ZooShell state={state} {...shellProps}><RefineBacklog state={state} onSetSprintDays={setSprintDays} onSetDod={setDod} onAgreeDod={agreeDod} onEstimate={estimate} onAddPbi={createPbi} onRefinePbi={refinePbi} onReorder={reorder} onMoveZone={moveZoneOrder} onMoveBefore={moveBefore} onSetUseStories={setUserStories} onSplitEpic={splitEpic} onChooseSolution={chooseSolution} onDeletePbi={deletePbi} onDuplicatePbi={duplicatePbi} onPlan={() => setPhase('planning')} onOpenGround={openGround} teachCard={cardFor('refine')} onMarkTaught={markTaught} /></ZooShell>;
      case 'planning':
      ...
      case 'sprint':
        return <ZooShell state={state} {...shellProps} tools={<BoardTools state={state} teachCard={cardFor('sprint')} onMarkTaught={markTaught}
          onSetScrumAt={setDailyScrumAt} onSetLearnMode={setLearnMode} onSetWipLimit={setWipLimit} onCancelSprint={cancelSprint} />}><SprintBoard onAskToCheck={askToCheck} state={state} rail={<ActionRail className="mt-2 h-[26%] shrink-0 overflow-y-auto" state={state} seat={seat} onAnswerPlacement={answerPlacement} onAnswerQuestion={answerQuestion} onOpen={deployComplete} onSplitEpic={splitEpic} />} onEstimate={estimate} onToggleTask={toggleTask} onFinishItem={finishItem} onStartItem={startItem} onReorderSprint={reorderSprint} onPull={pull} onDropFromSprint={dropFromSprint} onAnswerPlacement={answerPlacement} onOpen={deployComplete} onEndDay={endDay} onHoldDailyScrum={holdDailyScrum} onAnswerImpediment={answerImpediment} onSkipDailyScrum={skipDailyScrum} onStartDay={beginDay} onHoldRefinement={holdRefinement} onSplitEpic={splitEpic} building={buildingId} edit={edit} part={partFocus} drawing={drawing} onAddPbi={createPbi} onSetUserStories={setUserStories} onAssignDev={assignDev} onBuilding={selectOnPark} seat={seat} canBuild={!seat || seat === 'developer' || (covering ?? []).includes('developer')} teachCard={cardFor('sprint')} /></ZooShell>;
      case 'review':
