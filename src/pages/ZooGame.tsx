...
export default function ZooGame() {
  const { state, start, setPhase, setGoal, setSprintGoal, setDod, takeSignal, plan, estimate, setTasks, toggleTask, startItem, toggleGoalCritical, setSprintDays, setLearnMode, setEnclosureSize, setItemPos, createPbi, refinePbi, reorder, moveBefore, setUserStories, pull, build, editBuild, addAnotherPbi, open, closeDay, holdDailyScrum, skipDailyScrum, beginDay, nextSprint, reset } = useZooGame();
  ...
  const render = () => {
    switch (state.phase) {
      ...
      case 'refine':
        return <ZooShell state={state} onPlaceItem={setItemPos}><RefineBacklog state={state} onEstimate={estimate} onAddPbi={createPbi} onRefinePbi={refinePbi} onReorder={reorder} onMoveBefore={moveBefore} onSetUseStories={setUserStories} onPlan={() => setPhase('planning')} /></ZooShell>;
      case 'planning':
