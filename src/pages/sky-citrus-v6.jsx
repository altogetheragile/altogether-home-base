...
// Two-colour wordmark — one word, two colours
...
const TeamIllustration = ({ height = 180, bg = p.paleTeal, count = 3, label }) => {
  const heights = [100, 120, 108, 95, 115];
  const colors = [p.deepTeal, p.midTeal, p.orange, "#006060", "#338080"];
  ...
  return (
    <div style={{ height, background: bg, borderRadius: 16, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", padding: "0 24px 16px", position: "relative", overflow: "hidden" }}>
      {label && (
        <div style={{ position: "absolute", top: 12, left: 0, right: 0, textAlign: "center", color: p.midTeal, fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>
          Illustration · {label}
        </div>
      )}
...
const IllustrationSpot = ({ height = 180, label, bg = p.paleTeal, style = {} }) => (
  <div style={{ height, background: bg, borderRadius: 16, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, ...style }}>
    <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
      <circle cx="24" cy="16" r="8" fill={p.midTeal} opacity="0.5" />
      <ellipse cx="24" cy="36" rx="14" ry="8" fill={p.midTeal} opacity="0.35" />
      <circle cx="24" cy="16" r="5" fill={p.deepTeal} opacity="0.4" />
    </svg>
    <div style={{ color: p.midTeal, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", textAlign: "center", padding: "0 16px" }}>{label}</div>
  </div>
);

export default function SkyAndCitrusV6() {
  const isMobile = useIsMobile();
  const [menuOpen, setMenuOpen] = useState(false);
  const navLinks = ["Events", "Knowledge Base", "Coaching", "About", "Contact"];
  const events = [
    { type: "Course", title: "Scrum Master Certification", date: "Apr 10–11, 2025", location: "London Office", price: "£900", duration: "2 days", instructor: "Alun Davies-Baker" },
    { type: "Workshop", title: "Agile Fundamentals Workshop", date: "Mar 15, 2025", location: "Remote Zoom Room", price: "Free", duration: "1 day", instructor: "Alun Davies-Baker" },
    { type: "Masterclass", title: "Agile Leadership Masterclass", date: "Aug 1, 2025", location: "Remote Zoom Room", price: "£900", duration: "1 day", instructor: "Alun Davies-Baker" },
  ];
  const typeColors = { Course: p.deepTeal, Workshop: p.midTeal, Masterclass: p.orange };
    ...
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", background: p.white }}>
      ...
      <div style={{ background: p.white, borderBottom: `1px solid ${p.paleTeal}`, position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ padding: isMobile ? "0 20px" : "0 48px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          ...
          <div className="aa-nav-links" style={{ gap: 32 }}>
            {navLinks.map(item => (
              <span key={item} style={{ color: p.body, fontSize: 13, fontWeight: 500, cursor: "pointer" }}>{item}</span>
            ))}
          </div>
        ...
        <div className={`aa-mobile-menu${menuOpen ? " open" : ""}`} style={{
          flexDirection: "column",
          background: p.white,
          borderTop: `1px solid ${p.paleTeal}`,
          padding: "8px 0 16px",
        }}>
          {navLinks.map(item => (
            <span
              ...
              style={{ color: p.body, fontSize: 16, fontWeight: 500, padding: "14px 20px", cursor: "pointer", borderBottom: `1px solid ${p.paleTeal}` }}
            >
              {item}
            </span>
          ))}
      ...
      <div className="aa-section-pad" style={{ background: p.skyTeal }}>
        <div className="aa-hero-grid">
          <div>
            <h1 className="aa-hero-h1" style={{ color: p.deepTeal, fontWeight: 800, lineHeight: 1.15, margin: "0 0 20px" }}>
            ...
            <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
              <button style={{ background: p.orange, color: "#fff", border: "none", padding: "13px 26px", borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
                Browse Events <Icons.ArrowRight />
              </button>
              <span style={{ color: p.deepTeal, fontWeight: 700, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
                Knowledge Base <Icons.ArrowRight />
              </span>
            </div>
          </div>
          <div className="aa-hide-mobile" style={{ position: "relative" }}>
            <TeamIllustration height={280} bg={p.paleTeal} count={4} label="Diverse team" />
            <div style={{ position: "absolute", bottom: -12, right: -12, background: p.orange, color: "#fff", borderRadius: 12, padding: "10px 16px", fontSize: 12, fontWeight: 700, boxShadow: "0 4px 16px rgba(255,151,21,0.35)" }}>
      ...
      {/* STATS BAR */}
      <div className="aa-stats-bar" style={{ background: p.white, padding: "20px 48px", borderBottom: `1px solid ${p.paleTeal}` }}>
        {[
          ...
          <div key={i} style={{ textAlign: "center", padding: "8px 40px", borderRight: i < 3 ? `1px solid ${p.paleTeal}` : "none" }}>
            ...
            <div style={{ color: p.muted, fontSize: 12 }}>{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="aa-section-pad" style={{ background: p.skyTeal }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 32 }}>
          <div>
            <h2 style={{ color: p.deepTeal, fontSize: 32, fontWeight: 800, margin: "0 0 6px" }}>Upcoming Events</h2>
            <p style={{ color: p.muted, fontSize: 15, margin: 0 }}>Certified courses, workshops and masterclasses</p>
          </div>
          <span style={{ color: p.orange, fontWeight: 700, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>View all <Icons.ArrowRight /></span>
        </div>
        <div className="aa-three-col">
          {events.map((event, i) => (
            <div key={i} style={{ background: p.white, borderRadius: 14, padding: "24px", display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <span style={{ color: p.midTeal, fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
                  <Icons.Clock />{event.duration}
                </span>
              </div>
              <div style={{ color: p.deepTeal, fontSize: 17, fontWeight: 700, lineHeight: 1.3 }}>{event.title}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {[
                  { icon: <Icons.Calendar />, text: event.date },
                  { icon: <Icons.MapPin />, text: event.location },
                  { icon: <Icons.User />, text: event.instructor },
                ].map((row, j) => (
                  <div key={j} style={{ color: p.body, fontSize: 13, display: "flex", alignItems: "center", gap: 7 }}>
                    <span style={{ color: p.midTeal, flexShrink: 0 }}>{row.icon}</span>{row.text}
                  </div>
                ))}
              </div>
              <div style={{ marginTop: "auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ color: p.deepTeal, fontWeight: 800, fontSize: 18 }}>{event.price}</span>
                <button style={{ background: p.orange, color: "#fff", border: "none", padding: "9px 18px", borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div className="aa-section-pad" style={{ background: p.white }}>
        <div className="aa-two-col-wide">
          <div style={{ position: "relative" }}>
            <TeamIllustration height={320} bg={p.paleTeal} count={1} label="Alun - founder photo" />
            <div style={{ position: "absolute", bottom: -16, left: -16, background: p.deepTeal, color: "#fff", borderRadius: 12, padding: "12px 18px" }}>
              <div style={{ fontSize: 11, color: p.lightTeal, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>Credentials</div>
              {["ABC Level-4 Specialist", "Advanced Certified Scrum Master", "ABC Assessor", "University of Westminster Lecturer"].map((c, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#fff", marginBottom: 4 }}>
                  <span style={{ color: p.orange }}><Icons.CheckCircle /></span>{c}
                </div>
              ))}
            </div>
          </div>
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: p.skyTeal, color: p.midTeal, fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "5px 14px", borderRadius: 20, marginBottom: 20 }}>
              <Icons.User />About Alun
            </div>
            <h2 style={{ color: p.deepTeal, fontSize: 32, fontWeight: 800, margin: "0 0 16px", lineHeight: 1.2 }}>
              Train with someone<br />who's been in the room.
            </h2>
            <p style={{ color: p.body, fontSize: 15, lineHeight: 1.75, margin: "0 0 16px" }}>
            ...
            <p style={{ color: p.body, fontSize: 15, lineHeight: 1.75, margin: "0 0 28px" }}>
            ...
            <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
              <button style={{ background: p.orange, color: "#fff", border: "none", padding: "12px 24px", borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
                Book a chemistry session <Icons.ArrowRight />
              </button>
              <span style={{ color: p.deepTeal, fontWeight: 700, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div className="aa-section-pad" style={{ background: p.deepTeal }}>
        <div className="aa-two-col-right">
          <div>
            ...
            <p style={{ color: p.lightTeal, fontSize: 16, margin: "0 0 24px", lineHeight: 1.6 }}>
            ...
            <button style={{ background: p.orange, color: "#fff", border: "none", padding: "12px 24px", borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 8 }}>
      ...
      <div className="aa-section-pad" style={{ background: p.white }}>
        <div className="aa-two-col-left">
          <div>
            <h2 style={{ color: p.deepTeal, fontSize: 32, fontWeight: 800, margin: "0 0 12px" }}>What our attendees say</h2>
            <p style={{ color: p.muted, fontSize: 15, margin: "0 0 24px" }}>Real feedback from real practitioners</p>
            <TeamIllustration height={180} bg={p.skyTeal} count={3} label="Team group" />
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              ...
              <div key={i} style={{ background: p.skyTeal, borderRadius: 14, padding: "18px 20px", display: "flex", gap: 14, alignItems: "flex-start" }}>
                <div style={{ width: 38, height: 38, borderRadius: "50%", background: p.paleTeal, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", color: p.midTeal }}>
                  <Icons.User />
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ color: p.body, fontSize: 13, lineHeight: 1.6, marginBottom: 8 }}>"{t.quote}"</div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ color: p.deepTeal, fontWeight: 700, fontSize: 12 }}>{t.name}</div>
                      <div style={{ color: p.muted, fontSize: 11 }}>{t.role}</div>
                    </div>
                    <span style={{ display: "flex", alignItems: "center", gap: 4, background: p.paleTeal, color: p.midTeal, fontSize: 10, fontWeight: 700, padding: "3px 10px", borderRadius: 20 }}>
        ...
        <div className="aa-two-col-cta">
          <div>
            ...
            <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
              <button style={{ background: p.deepTeal, color: "#fff", border: "none", padding: "13px 28px", borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
                Browse Events <Icons.ArrowRight />
              </button>
              <span style={{ color: "rgba(255,255,255,0.95)", fontWeight: 700, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <div className="aa-footer-grid">
          <div>
            <div style={{ marginBottom: 16 }}>
              <LogoFull height={40} light={true} />
            </div>
            <div style={{ color: p.lightTeal, fontSize: 14, lineHeight: 1.75, maxWidth: 300 }}>
          ...
          <div>
            ...
            {["Events", "Knowledge Base", "Coaching", "About", "Contact"].map(link => (
              <div key={link} style={{ color: p.lightTeal, fontSize: 13, marginBottom: 8, cursor: "pointer" }}>{link}</div>
            ))}
          </div>
          <div>
