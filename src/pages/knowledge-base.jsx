...
const TechniqueImage = ({ category, index }) => {
...
const techniques = [
  { category: "Analysis", title: "Story Mapping", summary: "Visualise the full user journey across a horizontal backbone of activities, with stories sliced vertically by priority.", horizons: ["Team"], frameworks: ["Scrum", "AgileBA"], origins: "Jeff Patton, 2005" },
  { category: "Planning", title: "OKRs", summary: "Set bold intent (Objectives) and define measurable outcomes (Key Results) that cascade from strategic to team level.", horizons: ["Strategic", "Coordination"], frameworks: ["ICAgile", "AgileBA"], origins: "Intel / Google, 1970s–1990s" },
  { category: "Analysis", title: "Impact Mapping", summary: "A structured mind map linking business goals to actors, impacts, and deliverables — keeps delivery connected to outcomes.", horizons: ["Coordination", "Team"], frameworks: ["AgileBA"], origins: "Gojko Adzic, 2012" },
  { category: "Facilitation", title: "Retrospectives", summary: "A regular team ceremony to inspect working practices and identify one meaningful improvement for the next iteration.", horizons: ["Team"], frameworks: ["Scrum", "AgilePM"], origins: "Norm Kerth / Agile Manifesto, 2001" },
  { category: "Strategy", title: "Business Model Canvas", summary: "A one-page visual framework for designing, describing, and challenging business models across nine building blocks.", horizons: ["Strategic"], frameworks: ["ICAgile"], origins: "Osterwalder & Pigneur, 2010" },
  { category: "Analysis", title: "Personas", summary: "Fictional but evidence-based profiles of user types that anchor design and delivery decisions in real human needs.", horizons: ["Coordination", "Team"], frameworks: ["AgileBA", "ICAgile"], origins: "Alan Cooper, 1999" },
  { category: "Delivery", title: "Kanban Board", summary: "A visual workflow tool that makes work visible, limits work in progress, and exposes bottlenecks in the delivery system.", horizons: ["Team"], frameworks: ["AgilePM", "ICAgile"], origins: "Toyota Production System, 1950s" },
  { category: "Analysis", title: "5 Whys", summary: "An iterative root cause analysis technique — ask 'why' five times to move from symptom to systemic cause.", horizons: ["Team"], frameworks: ["AgileBA"], origins: "Sakichi Toyoda / Toyota, 1930s" },
  { category: "Planning", title: "MoSCoW Prioritisation", summary: "Classify requirements as Must have, Should have, Could have, or Won't have to drive sprint and release planning.", horizons: ["Coordination", "Team"], frameworks: ["AgilePM", "AgileBA"], origins: "Dai Clegg / DSDM, 1994" },
  { category: "Facilitation", title: "Event Storming", summary: "A large-group modelling technique using sticky notes to map domain events, commands, and aggregates across a system.", horizons: ["Coordination"], frameworks: ["AgileBA"], origins: "Alberto Brandolini, 2013" },
  { category: "Delivery", title: "Definition of Done", summary: "A shared team agreement on the criteria every increment must meet before it can be considered complete and releasable.", horizons: ["Team"], frameworks: ["Scrum", "AgilePM"], origins: "Scrum community, early 2000s" },
  { category: "Strategy", title: "Value Proposition Canvas", summary: "Zoom in on how a product or service creates gains and relieves pains for a specific customer segment.", horizons: ["Strategic", "Coordination"], frameworks: ["ICAgile"], origins: "Osterwalder et al., 2014" },
];
...
export default function KnowledgeBasePage() {
  ...
  const filtered = techniques
    ...
    .filter(t => t.title.toLowerCase().includes(search.toLowerCase()) || t.summary.toLowerCase().includes(search.toLowerCase()));
    ...
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", background: p.white }}>
        ...
        <div className="aa-page-intro" style={{ paddingBottom: isMobile ? 32 : 48, borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16, justifyContent: "flex-end" }}>
            ...
            <div style={{ display: "flex", gap: 24 }}>
              {[{ n: "80+", label: "Techniques" }, { n: "5", label: "Categories" }, { n: "6", label: "Frameworks" }].map((s, i) => (
                <div key={i}>
      ...
      <div className="aa-section-pad" style={{ background: p.skyTeal }}>
        <div className="aa-kb-grid">
          {filtered.map((tech, i) => {
            const col = categoryColours[tech.category] || categoryColours.Analysis;
            return (
              <div key={i} style={{ background: p.white, borderRadius: 14, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 2px 12px rgba(0,77,77,0.07)" }}>
                <TechniqueImage category={tech.category} index={i} />
                <div style={{ padding: "20px 20px 24px", flex: 1, display: "flex", flexDirection: "column", gap: 12 }}>

                  {/* category pill */}
                  <div>
                    <span style={{ background: col.pill, color: "#fff", fontSize: 10, fontWeight: 700, padding: "3px 10px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.05em" }}>{tech.category}</span>
                  </div>

                  {/* title */}
                  <div style={{ color: p.deepTeal, fontSize: 18, fontWeight: 800, lineHeight: 1.25 }}>{tech.title}</div>

                  {/* summary */}
                  <div style={{ color: p.muted, fontSize: 13, lineHeight: 1.65, flex: 1 }}>{tech.summary}</div>
                  ...
                  <div style={{ borderTop: `1px solid ${p.paleTeal}`, paddingTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                      ...
                      {tech.horizons.map(h => (
                        <span key={h} style={{ background: p.paleTeal, color: p.midTeal, fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20 }}>{h}</span>
                      ))}
                    </div>

                    {/* frameworks */}
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                      <span style={{ color: p.muted, fontSize: 11, fontWeight: 600 }}>Frameworks:</span>
                      {tech.frameworks.map(f => (
                        <span key={f} style={{ background: p.skyTeal, color: p.deepTeal, fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20, border: `1px solid ${p.paleTeal}` }}>{f}</span>
                      ))}
                    </div>

                    {/* origins */}
                    <div style={{ color: p.muted, fontSize: 11 }}>
                      <span style={{ fontWeight: 600 }}>Origins: </span>{tech.origins}
                    </div>
                  </div>

                  {/* CTA */}
                  <button style={{ marginTop: 4, background: "none", border: "none", padding: 0, color: p.deepTeal, fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, textAlign: "left" }}>
      ...
      <div style={{ background: "#006666", padding: isMobile ? "40px 20px" : "56px 48px" }}>
        <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 40, alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12, flexShrink: 0 }}>
            ...
            <button style={{ background: "none", border: "none", padding: 0, color: p.lightTeal, fontWeight: 700, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap" }}>
