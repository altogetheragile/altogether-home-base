...
export function ZooShell({ state, children, parkTab, onSetTab, links, menuLinks, backlogTab, onReading, onCommitBuild, onTurn, onSetMemberSpot, onMoveInside, onSetClockPaused,  onWho, tools,  onPutIn, onAskToCheck, canBuild = true, building, onOpenBuild, edit,  drawRoute, drawing, onDrawing,  onPlaceItem,  onAddConnector, onUpdateConnector, onRemoveRun,         onSetSize, onAddCopy, onSetCopyPiece, onRemovePlant, onMoveCopy,      onSetDod, onSetDor, onSetProductGoal, onSave, onOpenSaves, onPoRefine, poRefining, poNote, onDismissPoNote, said, onDismissSaid, refused, onDismissRefused, onMarkTaught, onBack, copy, seat = null, observer, covering, away }: { state: ZooGameState; children: ReactNode; onPart?: (p: { id: string; key: string } | null) => void; drawRoute?: { id: string; name: string; style: { thickness: number; color: string } } | null; drawing?: boolean; onDrawing?: (on: boolean) => void; parkTab?: ArtifactTab; onSetTab?: (t: ArtifactTab) => void;
  ...
  // The next thing worth explaining here, if it has not been read yet.
  ...
  const teachCard = !['refine', 'planning', 'sprint', 'review', 'retro'].includes(state.phase)
      ...
      <header className="shrink-0 border-b border-border px-2 pt-1.5 sm:px-3">
        ...
        <div className="zoo-band -mx-2 mb-1.5 flex items-center gap-3 px-2 py-2 sm:-mx-3 sm:px-3">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            ...
            {/* What the zoo is worth to the people who come to it: one for every visit that was
                worth making. Not money and not points - a forecast buys nothing, which is the rule
                this counter exists to make visible. */}
            <span data-part="value" title="What the zoo is worth to its visitors: one for every visit worth making, less welfare penalties"
              className="flex shrink-0 items-center gap-1 rounded-full bg-white/10 px-2.5 py-1 text-sm font-bold text-emerald-200">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            ...
            <LearnDrawer state={state} notes={notes} onReading={onReading}
              ...
              <div className="relative flex min-h-0 min-w-0 shrink-0 flex-col rounded-lg border-2 border-border bg-card p-2 xl:shrink">
                ...
                {edit && canBuild && (
                  <ParkOptions className="mb-2 shrink-0 border-b border-border pb-2" state={state}
                    ...
                    api={{
                      ...
                      onSetServices: edit.onSetServices,
                      onTurn,
                      onUnplace: (id) => setMoving(id),
                      onPutIn,
                      // The first act of building, on the same toolbar as everything that follows
                      // it: one row, in the order the work happens.
                      onChooseStructure: edit.onChooseStructure,
                      onRemoveInside: edit.onRemoveInside,
                      ...
                      // A planting item is a clump: another plant, what each one is, and taking one
                      // out. The park has always drawn and moved them; this is where they arrive.
                      onAddCopy,
                      onSetCopyPiece,
                      onRemovePlant,
                      onRemoveRun,
                ...
                <div className="relative min-h-0 flex-1 overflow-hidden"
                  ...
                  <ParkPlan state={state} height={620} selected={building ?? null} inside={inside}
                    ...
                    onPlaceItem={onPlaceItem} onSetSize={onSetSize} onTurn={onTurn} onSetMemberSpot={onSetMemberSpot} onMoveInside={onMoveInside} onMoveCopy={onMoveCopy}
                    ...
                    // Laying a path bends the run it is carrying on from, and a joint can be
                    // dragged afterwards. Both go through the same update.
