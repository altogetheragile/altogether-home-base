...
export function ZooShell({ state, children, parkTab, onSetTab, onPlaceItem, onSetPathStyle, onSetPathRoute, onAddPath, onDeletePath, onClearPaths, onImprove, onSetSpot, onNest, onUnnest, onEndDay, onSetDod, onSetProductGoal, onSave, onOpenSaves, onPoRefine, poRefining, poNote, onDismissPoNote }: { state: ZooGameState; children: ReactNode; parkTab?: 'work' | 'park'; onSetTab?: (t: 'work' | 'park') => void; onPlaceItem?: (id: string, pos: { x: number; y: number }) => void; onSetPathStyle?: (key: string) => void; onSetPathRoute?: (route: 'straight' | 'elbow' | 'spine' | 'none') => void; onAddPath?: (points: { x: number; y: number }[]) => void; onDeletePath?: (id: string) => void; onClearPaths?: () => void; onImprove?: (id: string) => void; onSetSpot?: (id: string, spot: { x: number; y: number }) => void; onNest?: (id: string, enclosureId: string, spot: { x: number; y: number }) => void; onUnnest?: (id: string) => void; onEndDay?: () => void; onSetDod?: (dod: string[]) => void; onSetProductGoal?: (goal: string) => void; onSave?: () => void; onOpenSaves?: () => void; onPoRefine?: () => void; poRefining?: boolean; poNote?: string | null; onDismissPoNote?: () => void }) {
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-hidden xl:flex">
        ...
        <div className={cn('h-full overflow-y-auto px-2 py-3 sm:px-3',
          ...
          <ParkView state={state} large onPlaceItem={onPlaceItem} onSetPathStyle={onSetPathStyle} onSetPathRoute={onSetPathRoute} onAddPath={onAddPath} onDeletePath={onDeletePath} onClearPaths={onClearPaths} onImprove={onImprove} onSetSpot={onSetSpot} onNest={onNest} onUnnest={onUnnest} />
        </div>
      </div>
    </div>
  );
