...
export function zooActions(send: (action: ZooAction) => void) {
  return {
    ...
    /** Take one of something back out of a habitat. */
    removeInside: (id: string, kind: string) => send({ type: 'REMOVE_INSIDE', id, kind }),
    ...
    removePlant: (id: string, index: number) => send({ type: 'REMOVE_PLANT', id, index }),
    ...
    askToCheck: (id: string) => send({ type: 'ASK_TO_CHECK', id }),
    ...
    setClockPaused: (paused: boolean) => send({ type: 'SET_CLOCK_PAUSED', paused }),
