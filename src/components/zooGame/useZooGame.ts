...
function reducer(state: ZooGameState, action: ZooAction): ZooGameState {
  switch (action.type) {
    ...
    case 'ADD_COPY':
      return addItemCopy(state, action.id, action.pos);
    case 'MOVE_COPY':
...
export function useZooGame(gameSeed?: number) {
  ...
  const addCopy = useCallback((id: string, pos: { x: number; y: number }) => dispatch({ type: 'ADD_COPY', id, pos }), []);
