...
export function BenefitsScorecardEditor({ initialData, artifactId, projectId }: BenefitsScorecardEditorProps = {}) {
  ...
  return (
    <div className="space-y-6">
      <style>{`.exporting .benefit-no-export { display: none !important; }`}</style>
            ...
            <div key={b.id} className="rounded-md border border-border p-4">
              ...
              <div className="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <input value={b.leading_indicator} onChange={(e) => updateBenefit(b.id, { leading_indicator: e.target.value })} placeholder="What you watch to see it coming" className="mt-1 w-full rounded-md border border-border bg-background p-2 text-sm" />
                </div>
                <div>
                  <label className="text-xs font-bold tracking-wide text-muted-foreground">TARGET</label>
                  <input value={b.target} onChange={(e) => updateBenefit(b.id, { target: e.target.value })} placeholder="e.g. under 3 days" className="mt-1 w-full rounded-md border border-border bg-background p-2 text-sm" />
                </div>
              </div>
