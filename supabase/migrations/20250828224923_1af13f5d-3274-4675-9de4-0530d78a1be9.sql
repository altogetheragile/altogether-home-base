-- Clean up database - keep only knowledge items from latest import (20250828_Agile_Planning_Activities_With_BMC.xlsx)

-- First, let's identify the latest import
DO $$
DECLARE
...
BEGIN
    ...
    WHERE NOT EXISTS (
        SELECT 1 FROM staging_data sd 
        WHERE sd.import_id = latest_import_id 
        AND sd.entity_id = knowledge_items.id::text
    );
    ...
    -- Step 5: Update import status
    UPDATE data_imports 
    SET 
        status = 'completed_with_cleanup',
    ...
    RAISE NOTICE 'Database cleanup completed. Removed % old knowledge items and related data.', old_items_count;
END $$;