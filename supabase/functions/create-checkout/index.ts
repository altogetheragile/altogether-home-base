...
serve(async (req) => {
  ...
  try {
    ...
    logStep("Event fetched", { eventTitle: event.title, price: event.price_cents });
    ...
    await supabaseService.from("event_registrations").insert({
      user_id: user.id,
      event_id: eventId,
    ...
    logStep("Registration record created");
